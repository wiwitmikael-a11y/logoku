...
const QuickActionsToolbar: React.FC<QuickActionsToolbarProps> = ({ layer, transform, onDelete, onDuplicate, onMoveUp, onMoveDown }) => {
  ...
  return (
    ...
    >
        <button title="Layer Up" onClick={onMoveUp} className="p-2.5 text-text-muted hover:bg-background rounded-md transition-colors"><svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M5 15l7-7 7 7" /></svg></button>
        <button title="Layer Down" onClick={onMoveDown} className="p-2.5 text-text-muted hover:bg-background rounded-md transition-colors"><svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" /></svg></button>
        <div className="w-px h-5 bg-border-main mx-1"></div>
        <button title="Duplicate" onClick={onDuplicate} className="p-2.5 text-text-muted hover:bg-background rounded-md transition-colors"><svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z" /></svg></button>
        <div className="w-px h-5 bg-border-main mx-1"></div>
        <button title="Delete" onClick={onDelete} className="p-2.5 text-red-500 hover:bg-red-500/10 rounded-md transition-colors"><svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg></button>
    </div>
  );
};
