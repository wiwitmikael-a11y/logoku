...
const DeleteProjectSliderModal: React.FC<Props> = ({ show, onClose, onConfirm, projectNameToDelete, projectLogoUrl, isConfirmLoading }) => {
  ...
  const keyframes = ` @keyframes paper-throw { 0% { transform: translate(0, 0) rotate(0deg) scale(1); opacity: 1; } 20% { transform: translate(15px, -30px) rotate(15deg) scale(1.1); } 100% { transform: translate(30px, 40px) rotate(90deg) scale(0.2); opacity: 0; } } .animate-paper-throw { animation: paper-throw 0.5s ease-in forwards; } @keyframes trash-shake { 0%, 100% { transform: translateX(0) rotate(0); } 20% { transform: translateX(-3px) rotate(-5deg); } 40% { transform: translateX(3px) rotate(5deg); } 60% { transform: translateX(-3px) rotate(-5deg); } 80% { transform: translateX(3px) rotate(5deg); } } .animate-trash-shake { animation: trash-shake 0.4s ease-in-out; } `;
  ...
  useEffect(() => { if (show) modalRef.current?.focus(); else setTimeout(resetSlider, 300); }, [show, resetSlider]);
  
  const handleOverlayClick = (e: React.MouseEvent<HTMLDivElement>) => { if (e.target === e.currentTarget && !isConfirmLoading) onClose(); }
  ...
  return (
    ...
    >
      ...
      <div className={`relative max-w-md w-full bg-surface border rounded-2xl shadow-xl p-8 flex flex-col items-center transition-colors duration-300 ${isSolved ? 'border-red-500/50' : 'border-border-main'}`}>
        <img src={`${GITHUB_ASSETS_URL}Mang_AI.png`} alt="Mang AI" className="w-24 mb-4" style={{ imageRendering: 'pixelated' }} />
        <h2 id="delete-modal-title" className="text-2xl font-bold text-red-500 mb-2">Hapus Project Ini?</h2>
        <p className="text-text-body mb-2 text-center"> Lo mau ngebuang project <strong className="text-text-header">"{projectNameToDelete}"</strong>? </p>
        <p className="text-xs text-text-muted mb-8 text-center">Awas, tindakan ini bakal ngapus semua data secara permanen dan gak bisa dibalikin lagi.</p>

        <div ref={trackRef} className="w-full h-16 bg-background rounded-full flex items-center p-2 relative border border-border-main">
          <div className="absolute left-0 top-0 h-full bg-red-500/20 rounded-full" style={{ width: `${sliderLeft + 50}px` }} />
          <div ref={sliderRef} onMouseDown={handleDragStart} onTouchStart={handleDragStart} className={`w-12 h-12 bg-red-600 rounded-full absolute flex items-center justify-center cursor-grab active:cursor-grabbing select-none shadow-lg ${isSolved ? 'opacity-0' : ''}`} style={{ left: `${sliderLeft}px` }} role="slider" aria-valuemin={0} aria-valuemax={100} aria-valuenow={Math.round((sliderLeft / getTrackBounds().maxSliderLeft) * 100) || 0}>
             <div ref={paperRef}>
                {projectLogoUrl ? <img src={projectLogoUrl} alt="Logo" className="w-8 h-8 object-contain bg-white p-1 rounded-md" /> : <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>}
             </div>
          </div>
          <span className={`text-center w-full font-semibold transition-opacity duration-300 ${(isDragging || isSolved) ? 'opacity-0' : 'opacity-100 text-text-muted'}`}> Geser untuk Hapus </span>
          <div className="absolute right-3 top-1/2 -translate-y-1/2 text-text-muted">
             <svg ref={trashRef} xmlns="http://www.w3.org/2000/svg" className="h-8 w-8" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M9 2a1 1 0 00-.894.553L7.382 4H4a1 1 0 000 2v10a2 2 0 002 2h8a2 2 0 002-2V6a1 1 0 100-2h-3.382l-.724-1.447A1 1 0 0011 2H9zM7 8a1 1 0 011 1v6a1 1 0 11-2 0V9a1 1 0 011-1zm4 0a1 1 0 011 1v6a1 1 0 11-2 0V9a1 1 0 011-1z" clipRule="evenodd" /></svg>
           </div>
        </div>
        {isConfirmLoading && !isSolved && <p className="text-accent text-sm mt-4">Menghapus...</p>}
        <div className="mt-8">
            <Button onClick={onClose} variant="secondary" disabled={isConfirmLoading}> Gak Jadi, Sayang </Button>
        </div>
      </div>
    </div>
  );
