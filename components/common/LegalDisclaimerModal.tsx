  ...
  return (
    ...
    >
      <div className="relative max-w-lg w-full bg-surface rounded-2xl shadow-xl p-8 flex flex-col items-center">
        ...
        <h2 id="disclaimer-title" className="text-3xl font-bold text-primary mb-4 text-center" style={{ fontFamily: 'var(--font-display)' }}> Penting! Sebelum Pakai Logo Ini </h2>
        ...
        <div className="flex items-start space-x-3 bg-background p-4 rounded-lg w-full max-w-md mb-6">
            <input type="checkbox" id="disclaimer-agree" checked={isChecked} onChange={() => setIsChecked(!isChecked)} className="h-5 w-5 mt-0.5 flex-shrink-0 rounded bg-surface border-border-main text-splash focus:ring-splash cursor-pointer" />
        ...
        <div className="flex gap-4">
            ...
            <Button onClick={handleConfirm} disabled={!isChecked} variant="splash"> Setuju & Lanjutkan </Button>
        </div>
      </div>
    </div>
  );
