  ...
  return (
    <div className="flex flex-col gap-8">
      <div className="text-center">
        <h2 className="text-2xl md:text-3xl font-bold text-sky-600 mb-2">Langkah 8: Rencana Konten Sosmed</h2>
        <p className="text-slate-600 max-w-3xl mx-auto">Stop bingung mau posting apa. Mang AI akan meriset tren terbaru di Google untuk membuatkan draf kalender konten selama 7 hari, lengkap dengan ide, caption, dan hashtag yang relevan.</p>
      </div>
      ...
      {calendar.length > 0 && (
        <div className="bg-orange-50 border border-orange-200 rounded-lg p-4 flex items-start gap-4 text-left">
            ...
            <div>
                ...
                <p className="text-sm text-orange-800 mt-1">Aset visual ini hanya disimpan sementara di browser. Segera selesaikan dan lanjutkan untuk menyimpan progres. <strong>Progres akan hilang jika lo me-refresh atau menutup halaman ini.</strong></p>
            </div>
        </div>
      )}
      ...
      {calendar.length > 0 && (
        <div ref={resultsRef} className="flex flex-col gap-6 scroll-mt-24">
          <h3 className="text-xl font-bold text-center text-slate-800">Draf Kalender Konten Lo:</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {calendar.map((item, index) => (
              <Card key={index} title={<><span className="font-normal text-slate-500">{item.hari} - </span>{item.tipe_konten}</>} className="relative animate-item-appear" style={{animationDelay: `${index*100}ms`}}>
                 ...
                 <div className="space-y-4">
                    <div>
                        <h4 className="font-semibold text-slate-800 text-sm mb-1">Ide Konten:</h4>
                        <p className="text-slate-600 text-sm selectable-text">{item.ide_konten}</p>
                    </div>
                    <div className="relative border-t border-slate-200 pt-3">
                         <h4 className="font-semibold text-slate-800 text-sm mb-1">Draf Caption:</h4>
                         <p className="text-slate-600 whitespace-pre-wrap text-sm pr-10 selectable-text">{item.draf_caption}</p>
                         <CopyButton textToCopy={item.draf_caption} className="absolute top-2 right-0"/>
                    </div>
                    <div className="relative border-t border-slate-200 pt-3">
                        <h4 className="font-semibold text-slate-800 text-sm mb-1">Hashtag:</h4>
                        <p className="text-sky-600 text-xs break-words pr-10 selectable-text">{item.rekomendasi_hashtag.join(' ')}</p>
                        <CopyButton textToCopy={item.rekomendasi_hashtag.join(' ')} className="absolute top-2 right-0"/>
                    </div>
                    <div className="border-t border-slate-200 pt-3">
                        <h4 className="font-semibold text-slate-800 text-sm mb-2">Aset Visual Postingan</h4>
                        {item.imageUrl ? (
                            <div className="bg-slate-100 p-2 rounded-lg aspect-square flex justify-center items-center cursor-pointer group" onClick={() => setModalImageUrl(item.imageUrl!)}>
          ...
          {sources.length > 0 && (
             <div className="mt-4">
                <h4 className="font-semibold text-slate-800 mb-2">Sumber dari Google Search:</h4>
                <div className="flex flex-wrap gap-2">
                    {sources.map((source, i) => source.web && (<a href={source.web.uri} key={i} target="_blank" rel="noopener noreferrer" className="bg-slate-100 hover:bg-slate-200 text-slate-600 text-xs font-medium px-2.5 py-1 rounded-full transition-colors">{source.web.title}</a>))}
                </div>
             </div>
          )}

          <div className="self-center mt-4 relative">
             {showNextStepNudge && (<CalloutPopup className="absolute bottom-full mb-2 w-max animate-fade-in">Ide konten siap! Lanjut, bro?</CalloutPopup>)}
