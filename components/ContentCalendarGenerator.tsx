  ...
  return (
    <div className="flex flex-col gap-8">
      <div className="text-center">
        <h2 className="text-4xl md:text-5xl font-bold text-splash mb-2">Langkah 8: Rencana Konten Sosmed</h2>
      ...
      {calendar.length > 0 && (
        <div className="bg-orange-400/10 border border-orange-400/20 rounded-lg p-4 flex items-start gap-4 text-left">
            ...
            <div>
                ...
                <p className="text-sm text-orange-300/80 mt-1">Aset visual ini hanya disimpan sementara di browser. Segera selesaikan dan lanjutkan untuk menyimpan progres. <strong>Progres akan hilang jika lo me-refresh atau menutup halaman ini.</strong></p>
            </div>
        </div>
      )}
      ...
      {calendar.length > 0 && (
        <div ref={resultsRef} className="flex flex-col gap-6 scroll-mt-24">
          <h3 className="text-3xl font-bold text-center text-text-header">Draf Kalender Konten Lo:</h3>
          ...
          {sources.length > 0 && (
             <div className="mt-4">
                ...
                <div className="flex flex-wrap gap-2">
                    {sources.map((source, i) => source.web && (<a href={source.web.uri} key={i} target="_blank" rel="noopener noreferrer" className="bg-background hover:bg-border-main text-text-body text-xs font-medium px-2.5 py-1 rounded-full transition-colors">{source.web.title}</a>))}
                </div>
             </div>
          )}
