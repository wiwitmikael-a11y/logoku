  ...
  return (
    <div className="flex flex-col gap-8">
      <div className="text-center">
        <h2 className="text-4xl md:text-5xl font-bold text-splash mb-2">Langkah 7: Studio Media Cetak</h2>
      ...
      <div className="flex flex-wrap border-b border-border-main">
          <button onClick={() => handleTabClick('banner')} className={`px-4 py-3 text-sm md:px-6 md:text-base font-semibold transition-colors ${activeTab === 'banner' ? 'text-splash border-b-2 border-splash' : 'text-text-muted hover:text-text-header'}`}>Spanduk (Horizontal 3:1)</button>
          <button onClick={() => handleTabClick('roll_banner')} className={`px-4 py-3 text-sm md:px-6 md:text-base font-semibold transition-colors ${activeTab === 'roll_banner' ? 'text-splash border-b-2 border-splash' : 'text-text-muted hover:text-text-header'}`}>Roll Banner (Vertikal 1:3)</button>
      </div>
      ...
      {designs.length > 0 && (
        <div className="bg-orange-400/10 border border-orange-400/20 rounded-lg p-4 flex items-start gap-4 text-left">
            ...
            <div>
                ...
                <p className="text-sm text-orange-300/80 mt-1">Aset visual ini hanya disimpan sementara di browser. Segera selesaikan dan finalisasi untuk menyimpan progres. <strong>Progres akan hilang jika lo me-refresh atau menutup halaman ini.</strong></p>
            </div>
        </div>
      )}
      ...
      {designs.length > 0 && (
        <div ref={resultsRef} className="flex flex-col gap-6 items-center scroll-mt-24">
            <h3 className="text-3xl font-bold text-text-header">Desain Hasil Generate:</h3>
