                        ...
                        <div className="crt-screen p-4 sm:p-6 flex flex-col gap-4 overflow-y-auto">
                            ...
                            <div className="flex border-b-2 border-cyan-400/50">
                                <button 
                                    ...
                                    className={`flex-1 font-mono font-bold py-2 text-sm sm:text-base transition-colors ${activeTool === 'name' ? 'bg-cyan-400/20 text-cyan-300' : 'text-gray-500 hover:bg-cyan-900/20'}`}
                                >
                                    NAME GEN
                                </button>
                                <button 
                                    onClick={() => handleToolChange('slogan')}
                                    className={`flex-1 font-mono font-bold py-2 text-sm sm:text-base transition-colors ${activeTool === 'slogan' ? 'bg-cyan-400/20 text-cyan-300' : 'text-gray-500 hover:bg-cyan-900/20'}`}
                                >
                                    SLOGAN GEN
                                </button>
                            </div>
                                ...
                                {activeTool === 'name' ? (
                                    <div className="animate-content-fade-in space-y-4">
                                        <div className="space-y-2">
                                            ...
                                            <input value={nameCategory} onChange={(e) => setNameCategory(e.target.value)} placeholder="e.g., Kopi Susu Gula Aren" required className="w-full font-mono bg-black/50 border-2 border-cyan-400/50 rounded-none p-2 text-white focus:outline-none focus:border-cyan-400 focus:ring-2 focus:ring-cyan-400/50" />
                                        </div>
                                        <div className="space-y-2">
                                            <label className="text-cyan-400 font-bold text-sm block">KEYWORDS (OPTIONAL):</label>
                                            <input value={nameKeywords} onChange={(e) => setNameKeywords(e.target.value)} placeholder="e.g., senja, santai, modern" className="w-full font-mono bg-black/50 border-2 border-cyan-400/50 rounded-none p-2 text-white focus:outline-none focus:border-cyan-400 focus:ring-2 focus:ring-cyan-400/50" />
                                        </div>
                                    ...
                                    <div className="animate-content-fade-in space-y-4">
                                        <div className="space-y-2">
                                            <label className="text-cyan-400 font-bold text-sm block">BUSINESS NAME:</label>
                                            <input value={sloganBusinessName} onChange={(e) => setSloganBusinessName(e.target.value)} placeholder="e.g., Kopi Senja" required className="w-full font-mono bg-black/50 border-2 border-cyan-400/50 rounded-none p-2 text-white focus:outline-none focus:border-cyan-400 focus:ring-2 focus:ring-cyan-400/50" />
                                        </div>
                                        <div className="space-y-2">
                                            <label className="text-cyan-400 font-bold text-sm block">KEYWORDS (OPTIONAL):</label>
                                            <input value={sloganKeywords} onChange={(e) => setSloganKeywords(e.target.value)} placeholder="e.g., santai, temen ngobrol" className="w-full font-mono bg-black/50 border-2 border-cyan-400/50 rounded-none p-2 text-white focus:outline-none focus:border-cyan-400 focus:ring-2 focus:ring-cyan-400/50" />
                                        </div>
                                    ...
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-2">
                                        {displayedItems.map((item, index) => (
                                            <div key={index} className="flex items-center gap-2 animate-content-fade-in">
                                                ...
                                                <CopyButton textToCopy={item} className="!bg-black/50 !text-cyan-400 hover:!bg-cyan-900/50" />
                                            </div>
                                        ))}
                                    </div>
                                )}
                            </div>
                            <p className="text-center text-xs text-cyan-400/50">MANG AI SYSTEMS - READY PLAYER ONE - +{XP_REWARD} XP</p>
                        </div>
                    </div>
