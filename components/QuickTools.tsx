...
const AICreator: React.FC = () => {
    ...
    const handleGenerate = async () => {
        ...
        const toolMap: { [K in CreatorTool]?: { cost: number; handler: () => Promise<void> } } = {
            moodboard: { cost: MOODBOARD_COST, handler: async () => { if (!moodboardKeywords) throw new Error('KEYWORDS CANNOT BE EMPTY!'); const [textData, images] = await Promise.all([generateMoodboardText(moodboardKeywords), generateMoodboardImages(moodboardKeywords)]); setMoodboardResult({ ...textData, images }); } },
            ...
            mascot: { cost: MASCOT_COST, handler: async () => { if (!mascotPrompt) throw new Error('DESKRIPSI MASKOT TIDAK BOLEH KOSONG!'); const res = await generateMascot(mascotPrompt); setMascotResults(res); } },
        };
    ...
    const handleSaveToLemari = async () => {
        ...
        switch(activeTool) {
            case 'moodboard': asset_data = moodboardResult; name = `Moodboard: ${moodboardKeywords}`; break;
            ...
            case 'mascot': asset_data = { urls: mascotResults }; name = `Mascot: ${mascotPrompt}`; break;
        }
    ...
    return (
        <div className="flex flex-col gap-8 max-w-4xl mx-auto animate-content-fade-in">
            ...
            <div className="creative-console-wrapper">
                    ...
                    <div className="console-monitor-frame">
                        <div className="crt-screen p-4 sm:p-6 flex flex-col gap-4 overflow-y-auto">
                            <div className="flex border-b-2 border-splash/50 overflow-x-auto">
                                {toolsConfig.map(tool => ( <button key={tool.id} onClick={() => handleToolChange(tool.id as CreatorTool)} className={`flex items-center gap-1.5 flex-shrink-0 font-mono font-bold py-2 px-3 text-xs sm:text-base transition-colors ${activeTool === tool.id ? 'bg-splash/20 text-splash' : 'text-text-muted hover:bg-splash/10'}`}><span>{tool.icon}</span><span>{tool.label}</span></button> ))}
                            </div>
                            <div className="flex-grow space-y-4">{renderToolInputs()}</div>
                            <div className="flex-grow min-h-[200px] border-t-2 border-splash/30 pt-4 overflow-y-auto">{renderToolResults()}</div>
                        </div>
                    </div>
             ...
             {modalImageUrl && (<ImageModal imageUrl={modalImageUrl} altText="Generated Asset" onClose={() => setModalImageUrl(null)} />)}
        </div>
    );
};
