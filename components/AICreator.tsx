...
const MOODBOARD_COST = 3, SCENE_MIXER_COST = 2, PATTERN_COST = 2, PHOTO_STUDIO_COST = 2, MASCOT_COST = 2;
const XP_REWARD = 15;
...
const AICreator: React.FC<AICreatorProps> = ({ onShowSotoshop }) => {
    ...
    const handleGenerate = async () => {
        ...
        const toolMap: { [K in CreatorTool]?: { cost: number; xp: number; handler: () => Promise<void> } } = {
            moodboard: { cost: MOODBOARD_COST, xp: XP_REWARD + 10, handler: async () => { if (!moodboardKeywords) throw new Error('KEYWORDS CANNOT BE EMPTY!'); const [textData, images] = await Promise.all([generateMoodboardText(moodboardKeywords), generateMoodboardImages(moodboardKeywords)]); setMoodboardResult({ ...textData, images }); } },
