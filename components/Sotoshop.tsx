...
const Sotoshop: React.FC<SotoshopProps> = ({ show, onClose, profile, deductCredits, setShowOutOfCreditsModal, addXp }) => {
    ...
    // --- DRAWING LOGIC ---
    // [REDACTED FOR BREVITY - Assume full drawing logic exists here]
    ...
    const redrawCanvas = useCallback(() => { /* [REDACTED] Full Canvas drawing logic is complex, this is a simplified placeholder. The actual implementation will be provided. */ }, [drawCanvasContent, layers, backgroundColor, selectedLayerId, zoom, pan, snapGuides]);

    useEffect(() => { redrawCanvas(); }, [redrawCanvas]);

    // --- INTERACTION & AI FUNCTIONS (Simplified) ---
    // [REDACTED for brevity - Assume full implementations exist for pointer events, gestures, AI calls, and export]
        ...
        <div className={`p-3 space-y-2 ${isModal ? '' : 'flex-grow overflow-y-auto'}`}>
            {layers.slice().reverse().map(layer => (
                 <div key={layer.id} onClick={() => setSelectedLayerId(layer.id)} className={`flex items-center gap-2 p-2 rounded-md cursor-pointer ${selectedLayerId === layer.id ? 'bg-splash/20' : 'hover:bg-background'}`}>
                    {/* [REDACTED] Layer item UI (thumbnail, name, visibility/lock icons) */}
    ...
    const renderPanelModal = (panel: Panel) => (
        <div className="absolute inset-0 bg-surface z-20 flex flex-col animate-content-fade-in">
            ...
            <div className="flex-grow overflow-y-auto">
                ...
                {/* [REDACTED] Other panels like upload, elements */}
            </div>
        </div>
    );
    ...
    return (
      <div className="fixed inset-0 bg-background flex flex-col z-50 text-text-body" style={{ touchAction: 'none' }}>
        ...
        <main className="flex-grow flex relative overflow-hidden">
            ...
            {/* Desktop Panels */}
            {isDesktop && <aside className="w-72 flex-shrink-0 bg-surface border-l border-border-main flex flex-col text-sm">{/* [REDACTED] Desktop panel logic */}</aside>}
            ...
            {!isDesktop && (
                <>
                    ...
                    <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex items-center gap-2 bg-surface/80 backdrop-blur-md border border-border-main p-2 rounded-full shadow-lg">
                        <button title="Layers" onClick={() => setActiveMobilePanel('layers')} className="w-12 h-12 flex items-center justify-center rounded-full bg-background/50 hover:bg-border-light text-text-muted">{/* Layers Icon */}</button>
                        <button title="Properties" onClick={() => selectedLayerId && setActiveMobilePanel('properties')} className="w-12 h-12 flex items-center justify-center rounded-full bg-background/50 hover:bg-border-light text-text-muted disabled:opacity-50" disabled={!selectedLayerId}>{/* Properties Icon */}</button>
                        <button title="Tambah Teks" onClick={() => addLayer({ type: 'text', content: 'Teks Baru', x: 50, y: 50, width: 200, height: 50, font: 'Poppins', size: 48, color: '#FFFFFF', textAlign: 'left', name: 'Teks Baru', rotation: 0, isVisible: true, isLocked: false, opacity: 1, shadow: {offsetX: 0, offsetY: 0, blur: 0, color: '#000000'}, blendMode: 'source-over' })} className="w-12 h-12 flex items-center justify-center rounded-full bg-background/50 hover:bg-border-light text-text-muted">{/* Text Icon */}</button>
                        <button title="Generate AI" onClick={() => setActiveMobilePanel('ai_generate')} className="w-12 h-12 flex items-center justify-center rounded-full bg-splash text-white">{/* AI Icon */}</button>
                    </div>
