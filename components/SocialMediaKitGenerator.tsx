...
const SocialMediaKitGenerator: React.FC<Props> = ({ projectData, onComplete, userId, projectId }) => {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
        ...
        const resultAssetsBase64 = await generateSocialMediaKitAssets(payload);
        
        // Upload both assets to Supabase Storage
        const [profilePicUrl, bannerUrl] = await Promise.all([
            uploadImageFromBase64(resultAssetsBase64.profilePictureUrl, userId, projectId, 'profile-picture'),
            uploadImageFromBase64(resultAssetsBase64.bannerUrl, userId, projectId, 'banner')
        ]);
      
        await deductCredits(GENERATION_COST);
        const finalAssets: SocialMediaKitAssets = { profilePictureUrl: profilePicUrl, bannerUrl: bannerUrl };
  ...
  }, [projectData, credits, deductCredits, setShowOutOfCreditsModal, userId, projectId]);
