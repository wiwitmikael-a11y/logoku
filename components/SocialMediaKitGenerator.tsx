  ...
  return (
    <div className="flex flex-col gap-8 items-center">
      <div className="text-center">
        <h2 className="text-2xl md:text-3xl font-bold text-sky-600 mb-2">Langkah 4: Social Media Kit</h2>
        <p className="text-slate-600 max-w-3xl mx-auto">Tampilan profil itu penting! Di sini, Mang AI akan membuatkan foto profil dan banner header yang serasi untuk muka depan sosial media lo (Facebook, X, YouTube, dll.).</p>
      </div>
      ...
      {assets && (
        <div className="bg-orange-50 border border-orange-200 rounded-lg p-4 flex items-start gap-4 text-left w-full max-w-4xl">
      ...
      {assets && (
        <div ref={resultsRef} className="flex flex-col gap-8 items-center scroll-mt-24 w-full max-w-5xl">
            <h3 className="text-xl font-bold text-slate-800">Aset Visual Sosmed Lo:</h3>
          <div className="grid grid-cols-1 md:grid-cols-5 gap-8 w-full items-center">
              <div className="md:col-span-2 flex flex-col items-center gap-3 animate-item-appear">
                  <Card title="Foto Profil" className="w-full">
                    <div className="flex flex-col items-center gap-4">
                        <div className="bg-slate-100 rounded-full p-2 flex items-center justify-center w-48 h-48 cursor-pointer group" onClick={() => setModalImageUrl(assets.profilePictureUrl)}>
                        ...
                        <p className="text-xs text-center text-slate-500">Cocok untuk Instagram, TikTok, Facebook, WhatsApp, dll.</p>
                    </div>
                  </Card>
              </div>
              ...
              <div className="md:col-span-3 flex flex-col items-center gap-3 animate-item-appear" style={{animationDelay: '150ms'}}>
                  <Card title="Banner / Header" className="w-full">
                    <div className="flex flex-col items-center gap-4">
                        <div className="bg-slate-100 rounded-lg p-2 flex items-center justify-center w-full aspect-video cursor-pointer group" onClick={() => setModalImageUrl(assets.bannerUrl)}>
                            <img src={assets.bannerUrl} alt="Generated Banner" className="w-full object-contain group-hover:scale-105 transition-transform" />
                        </div>
                        <p className="text-xs text-center text-slate-500">Cocok untuk header Facebook, X, YouTube, dll.</p>
                    </div>
                  </Card>
              </div>
          </div>
