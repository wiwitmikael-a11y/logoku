  ...
  return (
    <div className="flex flex-col gap-8">
      <div className="text-center">
        <h2 className="text-4xl md:text-5xl font-bold text-splash mb-2">Langkah Terakhir: Mockup Merchandise</h2>
      ...
      <div className="flex flex-wrap border-b border-border-main">
          {merchandiseTypes.map(merch => (
             <button key={merch.id} onClick={() => handleTabClick(merch.id)} className={`px-4 py-3 text-sm md:px-6 md:text-base font-semibold transition-colors ${activeTab === merch.id ? 'text-splash border-b-2 border-splash' : 'text-text-muted hover:text-text-header'}`}>{merch.name}</button>
          ))}
      </div>
      ...
      {designs.length > 0 && (
        <div ref={resultsRef} className="flex flex-col gap-6 items-center scroll-mt-24">
            <h3 className="text-3xl font-bold text-text-header">Mockup Hasil Generate:</h3>
