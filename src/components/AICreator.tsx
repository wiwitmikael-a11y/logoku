...
type CreatorTool = 'photo_studio' | 'scene_mixer' | 'moodboard' | 'pattern' | 'mascot' | 'video';
...
const AICreator: React.FC<AICreatorProps> = ({ projects }) => {
    ...
    const renderActiveTool = () => {
        ...
        switch (activeTool) {
            ...
            case 'video': return <Suspense fallback={<LoadingMessage />}><VideoGenerator {...props} /></Suspense>;
            default: return <p>Pilih alat untuk memulai.</p>;
        }
    };

    const tools = [
        { id: 'video', name: 'Video Generator', icon: '🎬' },
